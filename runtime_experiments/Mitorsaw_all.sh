...
# Usage:
#   ./Mitorsaw_all.sh <input_folder> <ref_fa> <output_prefix_dir>
#
# Example:
#   ./Mitorsaw_all.sh ./bam_inputs ./rCRS.fasta ./results
...
runtime_log="${output_prefix_dir}/mitorsaw_all_runtime.tsv"
...
for input_file in "${bam_files[@]}"; do
  ...
  /usr/bin/time -f "elapsed=%E user=%U sys=%S maxrss_kb=%M exit=%x" \
    bash ./Mitorsaw.sh "$input_file" "$ref_fa" "${out_prefix}" \
    2> "$time_log"
  exit_code=$?
  set -e
