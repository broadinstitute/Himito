...
# Usage:
#   ./mtdnaserver.sh <input_folder> <ref_fa> <output_prefix_dir>
#
# Example:
#   ./mtdnaserver.sh ./bam_inputs ./rCRS.fasta ./results
...
runtime_log="${output_prefix_dir}/mtdnaserver_all_runtime.tsv"
...
for input_file in "${bam_files[@]}"; do
  ...
  echo "Processing: $input_file"
    cat > mtdnaserver.config <<EOF
params {
    project         = "runtime-mtserve2-fusion"
    files           = "${input_file}"
    output          = "${out_prefix}/results/"
...
EOF
  ...
  /usr/bin/time -f "elapsed=%E user=%U sys=%S maxrss_kb=%M exit=%x" \
    nextflow run genepi/mtdna-server-2 -r v2.1.16 -c mtdnaserver.config -profile docker \
    2> "$time_log"
  exit_code=$?
  set -e
