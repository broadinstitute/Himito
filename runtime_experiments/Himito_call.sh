...
# Usage:
#   ./Himito_call.sh <input_folder> <ref_fa> <output_prefix_dir>
#
# Example:
#   ./Himito_call.sh ./gfa_inputs ./rCRS.fasta ./results
...
runtime_log="${output_prefix_dir}/himito_filter_runtime.tsv"
echo -e "gfa_file\telapsed\tuser_sec\tsys_sec\tmax_rss_kb\texit_code" > "$runtime_log"

shopt -s nullglob
gfa_files=("$input_folder"/*.gfa)

if [[ ${#gfa_files[@]} -eq 0 ]]; then
  echo "No .gfa files found in: $input_folder" >&2
  exit 1
fi

for input_file in "${gfa_files[@]}"; do
  base="$(basename "$input_file" .gfa)"
