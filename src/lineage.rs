...
pub fn hamming_distance_matrix(matrix: &HaplotypeMatrix) -> DistMatrix {
    ...
    let m   = matrix.variants.len();
    let inv = if m > 0 { 1.0 / m as f64 } else { 0.0 };
    ...
    for i in 0..n {
        for j in (i + 1)..n {
            let diff = matrix.haplotypes[i]
                .profile
                .iter()
                .zip(&matrix.haplotypes[j].profile)
                .filter(|(&a, &b)| a != b)
                .count();
            let d = diff as f64 * inv;
            data[i][j] = d;
            data[j][i] = d;
        }
    }
...
pub fn neighbor_joining(dist: &DistMatrix, hap_matrix: &HaplotypeMatrix) -> Result<Tree> {
    ...
    let zero_profile = vec![0u8; hap_matrix.variants.len()];
...
pub fn write_haplotype_map(hap_matrix: &HaplotypeMatrix, path: &str) -> Result<()> {
    ...
    for hap in &hap_matrix.haplotypes {
        let n_mut: usize = hap.profile.iter().map(|&b| b as usize).sum();
