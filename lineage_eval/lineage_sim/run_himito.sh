...
# Lineage HF band (keep in sync with run_eval.sh / sweep_fpfn.sh / score_lineage.py).
...
[[ -n "$OUTDIR" && -n "$PROFILE" ]] || {
  echo "usage: --outdir DIR --profile {hifi,ont-r10} [--sample S] [--fp F] [--fn F] [--min-hf F] [--max-hf F] [--minimal-ac N] [--vaf V] [--pval P]" >&2
  exit 1
}
