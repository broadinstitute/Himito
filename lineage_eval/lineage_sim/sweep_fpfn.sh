...
[[ -n "$OUTDIR" ]] || { echo "usage: --outdir DIR [--profile P] [--fp-grid \"...\"] [--fn-grid \"...\"] [--mcmc-iterations N] [--mcmc-chains N]" >&2; exit 1; }
...
n_cells=$((n_fp * n_fn))
echo "sweep: ${n_cells} cells (${n_fp}×${n_fn}), MCMC ${MCMC_CHAINS}×${MCMC_ITERS}, himito=$HIMITO" >&2
