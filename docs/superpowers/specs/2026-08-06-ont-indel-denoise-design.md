...
Protection is **narrowed to alleles the strand gates did not reject**. An allele rejected by the per-strand count minimum or by the strand-bias binomial test is a suspected single-strand artifact — precisely what those gates exist to catch, and precisely what the denoiser is meant to remove — so it is never shielded, however frequent it appears. Only alleles rejected by the context floor (or kept outright) are protected.
